...
livestock <- livestock %>% add_row(`...1` = rep(NA, 5), .before = 1)

# "검증상태" 열 삭제
livestock <- livestock %>% select(-1)
...
# 제목 행 추가(기존 자료와 합치기 위해 동일한 갯수로 추가)
industry <- industry %>% add_row(`...1` = rep(NA, 4), .before = 1)
...
# 제목 행 추가(기존 자료와 합치기 위해 동일한 갯수로 추가)
landuse <- landuse %>% add_row(.before = 1)
...
# 제목 행 추가(기존 자료와 합치기 위해 동일한 갯수로 추가)
fishfarm <- fishfarm %>% add_row(`...1` = rep(NA, 2), .before = 1)
