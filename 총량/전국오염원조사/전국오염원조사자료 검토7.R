...
# 시군별 합계 합치기
population_sum <- bind_rows(population_sum, population_subtotal_2)
## *****************************************************************************
...
waterusage <- waterusage %>%
  ...
  mutate(
    ...
    # 주소코드 추가("리"가 없는 "동"의 경우 "법정리"칸에 "동"으로 추가)
    주소 = str_c(시군구, 법정동, ifelse(is.na(법정리), 법정동, 법정리), sep = " ")
  )
...
# 유역별 합계 합치기
waterusage_sum <- bind_rows(waterusage_sum, waterusage_subtotal_1)
...
# 시군별 합계 합치기
waterusage_sum <- bind_rows(waterusage_sum, waterusage_subtotal_2)
## *****************************************************************************
...
for (file in files) {
  ...
  livestock <- bind_rows(livestock, temp)
}
## *****************************************************************************
...
for (file in files) {
  ...
  temp %<>%
    select(3, 5, 7, 8, 9, 10, 11, 12, 23, 72, 81, 연도) %>%
    set_names(c(
      "휴업", "업소명", "시도", "시군구", "법정동", "법정리", "본번", "부번",
      "규모", "폐수발생량", "폐수방류량", "연도"
    ))
...
for (file in files) {
  print(file)
  temp <- read_excel(file.path(dir, file), skip = 2, col_names = F) %>%
    select(-1) %>%
    set_names(c(
      "시도", "시군구", "법정동", "법정리", "총면적", "전", "답", "과수원",
  ...
  landuse <- bind_rows(landuse, temp)
}
## *****************************************************************************
